...
(function($){
	var errorobj=null,// 指示当前验证失败的表单元素;
		...
		tipmsg={// 默认提示文字;
			 ...
			 * if($("#Validform_msg").length!=0){return false;} msgobj=$('<div
			 * id="Validform_msg"><div class="Validform_title">'+tipmsg.title+'<a
			 * class="Validform_close" href="javascript:void(0);">&chi;</a></div><div
			 * class="Validform_info"></div><div class="iframe"><iframe
			 * frameborder="0" scrolling="no" height="100%" width="100%"></iframe></div></div>').appendTo("body");//提示信息框;
			 * 
			 * msgobj.find("a.Validform_close").click(function(){ msgobj.hide();
			 * msghidden=true; if(errorobj){
			 * errorobj.focus().addClass("Validform_error"); } return false;
			 * }).focus(function(){this.blur();});
			 * 
			 * $(window).bind("scroll resize",function(){ if(!msghidden){ var
			 * left=($(window).width()-msgobj.width())/2; var
			 * top=($(window).height()-msgobj.height())/2; var
			 * topTo=(document.documentElement.scrollTop?document.documentElement.scrollTop:document.body.scrollTop)+(top>0?top:0);
			 * msgobj.animate({ left : left, top : topTo },{ duration:400 ,
			 * queue:false }); } }) }
			 */
	...
	$.fn.sgfmform=function(settings){
		...
		this.each(function(){
			...
			$this.find("[datatype]").blur(function(){
				...
				// alert($this.val())
				flag=$.fn.sgfmform.sn.checkform(settings,$(this),$this,settings.tiptype,"hide");
				...
				if(!flag){
					if($(this).attr("ignore")=="ignore" && ( $(this).val()=="" || $(this).val()==$(this).attr("tip") )){
						if(settings.tiptype==2){
							...
							// 验证成功后恢复提示信息
							$.fn.sgfmform.sn.showmsg(b_this.attr("passinfo")||tipmsg.r,settings.tiptype,{obj:$(this),type:2},"hide");
						}
						flag=true;
						return true;
					}
				...
				$this.find("[datatype]").each(function(){
					...
					if($(this).attr("ajaxurl")){
						if($(this).attr("valid")!="true"){
							...
							errorobj=thisobj;
							// errorobj.focus();
							// $.fn.sgfmform.sn.showmsg(thisobj.attr("valid") ||
							// tipmsg.v,settings.tiptype,{obj:thisobj});
							alertMsg({"content" : thisobj.attr("valid") || tipmsg.v, "type" : "3", "close" : true});
							// if(!msghidden || settings.tiptype==2){
								// setTimeout(function(){
								// //--无需延时触发失去焦点，点击提交直接触发，否则会提示两次
							...
							// alertMsg({"content" :
							// s_this.attr("passinfo")||tipmsg.r, "type" : "1",
							// "close" : true})
							flag=true;
						}
					}
...
//							console.info($this)
//							var jsonData = $.fn.sgfmform.sn.serializeObject($this);
//							console.info(JSON.stringify(jsonData))
							...
							$.ajax({
								...
								data: $this.serialize(),
								cache: false,
						        /*contentType: "application/json;charest=utf-8",// 必须声明contentType为application/json,否则后台使用@RequestBody标注的话无法解析参数*/
