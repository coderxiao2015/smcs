...
public class SimpleExecutiveFilter extends AuthorizationFilter {
	...
	protected static final String[] blackUrlPathPattern = new String[] { "*.aspx*", "*.asp*", "*.php*", "*.exe*", "*.jsp*", "*.pl*", "*.py*", "*.groovy*", "*.sh*", "*.rb*", "*.dll*", "*.bat*", "*.bin*", "*.dat*", "*.bas*", "*.c*", "*.cmd*", "*.com*", "*.cpp*", "*.jar*", "*.class*", "*.lnk*" };
	...
	protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object obj) throws Exception {
		...
		for (String pattern : blackUrlPathPattern) {
			/*
			 * if (PatternUtils.simpleMatch(pattern, reqUrl)) { log.error(new
			 * StringBuffer().append("unsafe request >>> ").append(
			 * " request time: ").append( new SimpleDateFormat(
			 * "yyyy-MM-dd HH:mm:ss").format(new Date())).append(
			 * "; request ip: ") .append(WebUtils.getClientIP()).append(
			 * "; request url: ").append(httpRequest.getRequestURI())
